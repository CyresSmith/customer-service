...
const inputs: Partial<InputProps>[] = [
    ...
    { name: 'source', type: 'text' },
    {
        name: 'comment',
        type: 'textarea',
        placeholder: 'Побажання клієнта, додаткова інформація, примітки адміністратора..',
    },
];
...
const ClientForm = ({ initialState, onSubmit, isLoading, type }: Props) => {
    ...
    const disabledReset: boolean =
        JSON.stringify(
            Object.fromEntries(
                Object.entries(state).filter(i => i[0] !== 'avatar' && i[0] !== 'updatedAt')
            )
        ) ===
        JSON.stringify(
            Object.fromEntries(
    ...
    const disabledSubmit: boolean =
        invalidFields?.length > 0 || disabledReset || (state.firstName === '' && state.phone === '')
            ? true
            : false;
