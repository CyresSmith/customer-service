...
const ChooseTime = ({ eventDate, eventDuration, eventTime, daySchedule, setEventTime }: Props) => {
    ...
    const enableHours = timeArray.filter(t => isTimeEnableForEvent(eventDate, eventDuration, t, timeArray[timeArray.length - 1]));
    ...
    return (
        <>
            <CustomFormSelect width="fit-content" handleSelect={handleTimeSelect} selectItems={forSelect} selectedItem={{value: eventTime}} />
        </>
    )
};
