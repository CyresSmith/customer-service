...
const ThirdStep = ({
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (serviceData.durationMinutes?.id) {
            duration = duration + minutesToMilliseconds(+serviceData.durationMinutes.id);
        }
    ...
    const stateChange = (e: ChangeEvent<HTMLInputElement>, id?: number) => {
        ...
        const newValue = Number.isNaN(+value) ? 0 : +value >= 999999 ? 999999 : +value;
    ...
    const stateSelect = (selected: SelectItem, fieldName?: string, id?: number) => {
        if (!isAdmin) return;
    ...
    const serviceUpdate = async () => {
        ...
        if (updateObj.durationMinutes?.id) {
            duration = duration + minutesToMilliseconds(+updateObj.durationMinutes.id);
        }
    ...
    const saveDisabled =
        ...
        serviceData.price === 0 ||
        (serviceData.durationHours?.id === 0 && serviceData.durationMinutes?.id === 0) ||
            ...
            <div>
                <List>
                    <SettingsBlock
                        ...
                        durationMinutesValue={serviceData?.durationMinutes || minutesArray[0]}
