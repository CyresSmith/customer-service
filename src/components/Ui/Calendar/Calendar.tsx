...
const Calendar = ({
  ...
  const isMondayFirstDay = isMonday(
    startOfMonth(noSwitcher ? date : monthDate)
  );
  ...
  const getFullMonthArrayForRender = () => {
    ...
    if (!isMondayFirstDay) {
      daysArray = eachDayOfInterval({
        start: subDays(
          thisMonthStart,
          isSundayFirstDay ? 6 : thisMonthStart.getDay() - 1
        ),
  ...
  const isDayDisabled = (day: Date): boolean => {
    return enableDays
      ? !JSON.stringify(enableDays).includes(JSON.stringify(day)) &&
          !isSameDay(today, day)
      : false;
  };
